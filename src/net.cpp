...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "groincoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"digitalocean", "107.170.85.68"}, //First Seed Node
    {NULL, NULL},
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. groincoin is probably already running."), addrBind.ToString().c_str());
        else
