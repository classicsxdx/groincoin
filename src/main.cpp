...
uint256 hashGenesisBlock("0x00000a026dbda906b7ef90ed2cc3ae92e806f3d0523c4cba16f648a96b868e61"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // groincoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "groincoin Signed Message:\n";
...
{
    // groincoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // groincoin: 1 day
static const int64 nTargetSpacing =  150; // groincoin: 150 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // groincoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // groincoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // groincoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000a026dbda906b7ef90ed2cc3ae92e806f3d0523c4cba16f648a96b868e61");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
         ...
         // Genesis block        
        const char* pszTimestamp = "fuck everything";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1401283054;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1157563;
        ...
        assert(block.hashMerkleRoot == uint256("0x832d04551cd0fba5c99f858df6ac8b5a332f321f0edcb41c25bfb06278368cf5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf6, 0xc6, 0xb6, 0xd6 }; // groincoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("groincoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("groincoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static groincoinMiner(CWallet *pwallet)
{
    printf("groincoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("groincoin-miner");
    ...
    try { loop {
        ...
        printf("Running groincoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("groincoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&groincoinMiner, pwallet));
}
